...
| `auto_validé` | Extraction — confiance ≥ 80 % | Validé automatiquement par le pipeline |
| `validé` | Action manuelle (révision inline ou import CSV) | Certifié par l'utilisateur — corrections tracées |

- `à_réviser` → formulaire inline dans l'onglet "À réviser" (`révisé_par = 'user'`, passe en `validé`)
- `auto_validé` → modifiable via le bouton ✎ dans le ledger (passe en `validé`, correction tracée)
